...
mp.add_key_binding(nil, "drag-to-pan", function(t)
	...
	mp.add_forced_key_binding("MOUSE_MOVE", "drag-to-pan-mouse-move", function()
		...
		local align = old_align_x + 2 * (mouse_pos.x - old_mouse_pos.x) / (dims.ml + dims.mr)
		mp.set_property("video-align-x", clamp(align, -1, 1))
		align = old_align_y + 2 * (mouse_pos.y - old_mouse_pos.y) / (dims.mt + dims.mb)
...
mp.add_key_binding(nil, "cursor-centric-zoom", function(t)
	...
	local command = (options.suppress_osd and "no-osd " or "") .. "add video-zoom " .. amount .. ";"
	...
	-- So video-align-x is:
	local align = 2 * (ml - mp.get_property_native("video-pan-x") * width) / (dims.w - width) - 1
	command = command .. "no-osd set video-align-x " .. clamp(align, -1, 1) .. ";"

	local mt = (dims.mt - y) * 2 ^ amount + y
	align = 2 * (mt - mp.get_property_native("video-pan-y") * height) / (dims.h - height) - 1
